...
class ScholarMindRuntimeService:
    ...
    async def initialize(self, config: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            # 创建符合Runtime规范的智能体
            self.agent = ScholarMindRuntimeAgent(name=self.service_config["name"])
    ...
    async def start(self) -> bool:
        ...
        try:
            cli_logger.info("🌐 启动ScholarMind Runtime服务...")
            ...
            async with self.create_runner() as runner:
                ...
                cli_logger.info("✅ ScholarMind Runtime服务启动成功")
...
def parse_arguments():
    ...
    # Runtime配置
    parser.add_argument("--host", default="localhost", help="Runtime服务主机地址 (默认: localhost)")

    parser.add_argument("--port", type=int, default=8080, help="Runtime服务端口 (默认: 8080)")

    # 论文处理参数（用于direct模式）
    parser.add_argument(
        "input", nargs="?", help="论文输入 (文件路径、URL或文本) - 仅用于direct模式"
    )
