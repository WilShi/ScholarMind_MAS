...
# 创建运行时日志记录器
runtime_logger = setup_logger('scholarmind.runtime_agent', level='INFO', log_file=None, console=True)
...
class ScholarMindRuntimeAgent(AgentScopeAgent):
    ...
    def __init__(
        self,
        name: str = "ScholarMindRuntimeAgent",
        model=None,
        **kwargs
    ):
        """
    ...
    def _validate_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "input_type" in request_data:
            ...
            if request_data["input_type"] not in valid_types:
                errors.append(f"无效的输入类型: {request_data['input_type']}，支持的类型: {valid_types}")
        ...
        if "user_background" in request_data:
            ...
            if request_data["user_background"] not in valid_backgrounds:
                errors.append(f"无效的用户背景: {request_data['user_background']}，支持的背景: {valid_backgrounds}")
        
        # 使用工作流验证输入
        if "paper_input" in request_data and "input_type" in request_data and "user_background" in request_data:
