...
class ScholarMindAgentBase(AgentBase):
    ...
    def __init__(self, name: str, sys_prompt: str, model_config_name: Optional[str] = None, **kwargs):
        # 调用父类初始化（无参数）
        super().__init__()
    ...
    async def _ensure_model_initialized(self):
        ...
        if self.model is None:
            try:
                ...
                agent_logger.info(f"智能体 {self.name} 模型初始化成功: {model_config.get('model_name')}")
                
            except Exception as e:
                agent_logger.error(f"智能体 {self.name} 模型初始化失败: {e}")
                raise
        
    async def _safe_model_call(self, messages: list, fallback_response: Dict[str, Any] = None) -> Dict[str, Any]:
        """安全的模型调用，包含错误处理和重试机制"""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                agent_logger.warning(f"模型调用失败 (尝试 {attempt + 1}/{max_retries}) {self.name}: {e}")
...
class ScholarMindReActAgent(ReActAgent, ScholarMindAgentBase):
    ...
    def __init__(self, name: str, sys_prompt: str, toolkit=None, model_config_name: Optional[str] = None, **kwargs):
        # 获取模型配置
        from config import get_model_config
