...
"""
    ...
    async def test_chinese_filename_workflow(self):
        ...
        try:
            ...
            self.assertTrue(
                validation_result["valid"], f"输入验证失败: {validation_result['errors']}"
            )
    ...
    async def test_mixed_language_path_workflow(self):
        ...
        try:
            # 测试输入验证
            validation_result = self.pipeline.validate_inputs(str(file_path), "file", "advanced")
            self.assertTrue(
                validation_result["valid"], f"输入验证失败: {validation_result['errors']}"
            )
...
""",
        ...
        try:
            # 测试输入验证
            validation_result = self.pipeline.validate_inputs(str(file_path), "file", "beginner")
            self.assertTrue(
                validation_result["valid"], f"输入验证失败: {validation_result['errors']}"
            )
...
class TestChinesePathPerformance(unittest.TestCase):
    ...
    def test_path_operation_performance(self):
        ...
        # 创建测试文件
        test_files = [f"测试文件_{i}.txt" for i in range(10)]
        ...
        self.logger.info(
            f"路径存在性检查性能: {len(file_paths)}个文件耗时 {existence_check_time:.3f}秒"
        )
        ...
        self.logger.info(
            f"文件信息获取性能: {len(file_paths)}个文件耗时 {info_extraction_time:.3f}秒"
        )
